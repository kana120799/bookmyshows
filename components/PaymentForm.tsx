...
export default function PaymentForm({
  ...
  useEffect(() => {
    ...
    fetch("/api/stripe/checkout", {
      ...
      body: JSON.stringify({ amount, userId, bookingKey }),
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const { error, paymentIntent } = await stripe.confirmPayment({
        ...
        clientSecret,
        confirmParams: {
          return_url: `${window.location.origin}/payment-success?bookingKey=${bookingKey}&amount=${amount}`,
        },
      ...
      if (paymentIntent?.status === "succeeded") {
        ...
        window.location.href = `${window.location.origin}/payment-success?bookingKey=${bookingKey}&amount=${amount}`;
      }
    } catch (err) {
    ...
    <div className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          ...
          className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-4 rounded-lg disabled:opacity-70 disabled:cursor-not-allowed transition-colors duration-200 flex justify-center items-center"
        >
          {processing ? (
            <>
              <svg
